...
{
	if (IsValid(m_OwnerWeapon))
	{
		FProjectileType& projectileData = m_OwnerWeapon->ProjectileType;

		if (projectileData.Radius <= 0.001f)
		{
			if (IsValid(OtherActor) && IsValid(Hit.GetComponent()))
			{
				UGameplayStatics::ApplyPointDamage(OtherActor, 
					projectileData.Damage, 
					GetActorForwardVector(), 
					Hit, 
					m_OwnerWeapon->Character->GetController(), 
					this, 
